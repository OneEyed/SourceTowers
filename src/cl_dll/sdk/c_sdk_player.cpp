...
BEGIN_RECV_TABLE_NOBASE( C_SDKPlayer, DT_SDKLocalPlayerExclusive )
	...
	RecvPropInt( RECVINFO( m_iCommanderMode ) ),
	RecvPropEHandle( RECVINFO(m_hSelected) ),
END_RECV_TABLE()
...
BEGIN_PREDICTION_DATA( C_SDKPlayer )
	...
	DEFINE_PRED_FIELD( m_iCommanderMode, FIELD_INTEGER, FTYPEDESC_INSENDTABLE ),
	DEFINE_PRED_FIELD( m_hSelected, FIELD_EHANDLE, FTYPEDESC_INSENDTABLE ),
END_PREDICTION_DATA()
	...
	m_iv_angEyeAngles( "C_SDKPlayer::m_iv_angEyeAngles" )
{
	...
	//m_bCommanderMode = false;
	m_iCommanderMode = 2;

	m_bIsMultiSelect = false;
	m_vMouseDirection.Init();

//	m_iSelectedEntIndex = -1;
	//m_iHoveredEntIndex = -1;
}
...
int C_SDKPlayer::DrawModel( int flags )
{
	if(this != C_BasePlayer::GetLocalPlayer())
	{
		...
		AngleVectors( angle, &dest, NULL, NULL );
		//EyeVectors( &dest );
		dest = origin + (dest * MAX_TRACE_LENGTH);
		trace_t trace;
		CTraceFilterWORLDSPECIAL filter(this);
		UTIL_TraceLine( origin, dest, MASK_SOLID_BRUSHONLY, &filter, &trace );
		dest = trace.endpos;

		color32 color = {255,255,255,255};

		IMaterial *pMaterial = materials->FindMaterial( "sprites/bluelaser1", TEXTURE_GROUP_CLIENT_EFFECTS );
		FX_DrawLine( origin, dest, 3.0, pMaterial, color );
		//debugoverlay->AddLineOverlay(origin, dest, 255, 0, 0, true, 0);
		*/

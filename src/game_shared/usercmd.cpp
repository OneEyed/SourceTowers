...
void WriteUsercmd( bf_write *buf, CUserCmd *to, CUserCmd *from )
{
	...
	//---------------------------------------------
	//TOWERS CODE - Mouse Pointer angles
	//Single Click
	if ( to->mousedirection[ 0 ] != from->mousedirection[ 0 ] )
	{
		buf->WriteOneBit( 1 );
		buf->WriteBitAngle( to->mousedirection[ 0 ], 16 );
	}
	else
		buf->WriteOneBit( 0 );

	if ( to->mousedirection[ 1 ] != from->mousedirection[ 1 ] )
	{
		buf->WriteOneBit( 1 );
		buf->WriteBitAngle( to->mousedirection[ 1 ], 16 );
	}
	else
		buf->WriteOneBit( 0 );

	if ( to->mousedirection[ 2 ] != from->mousedirection[ 2 ] )
	{
		buf->WriteOneBit( 1 );
		buf->WriteBitAngle( to->mousedirection[ 2 ], 16 );
	}
	else
		buf->WriteOneBit( 0 );
	//END Tower code
...
void ReadUsercmd( bf_read *buf, CUserCmd *move, CUserCmd *from )
{
	...
	*move = *from;
	...
	//-------------------------------------------------------
	//TOWERS CODE - Mouse Point Angles
	if ( buf->ReadOneBit() )
		move->mousedirection[0] = buf->ReadBitAngle( 16 );
	if ( buf->ReadOneBit() )
		move->mousedirection[1] = buf->ReadBitAngle( 16 );
	if ( buf->ReadOneBit() )
		move->mousedirection[2] = buf->ReadBitAngle( 16 );

	//If we're doing a multi select, grab our ents unique ids!
	if ( buf->ReadOneBit() )
	{
		move->ismultiselect = true;

		int i;
		int count = buf->ReadShort();
		
		move->selectedtargets.RemoveAll();
		move->selectedtargets.SetCount( count );

		for (i = 0; i < count; i++)
		{
			move->selectedtargets[i] = buf->ReadUBitLong(24);
		}
	}

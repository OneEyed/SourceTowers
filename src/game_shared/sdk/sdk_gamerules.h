//========= Copyright © 1996-2005, Valve Corporation, All rights reserved. ============//
//
// Purpose: The TF Game rules object
//
// $Workfile:     $
// $Date:         $
// $NoKeywords: $
//=============================================================================//

#ifndef SDK_GAMERULES_H
#define SDK_GAMERULES_H

#ifdef _WIN32
#pragma once
#endif


#include "teamplay_gamerules.h"
#include "convar.h"
#include "gamevars_shared.h"

#ifdef CLIENT_DLL
	#include "c_baseplayer.h"
#else
	#include "player.h"
#include "sdk_player.h"
#include "towers/mapfilter.h"

#endif


#ifdef CLIENT_DLL
	#define CSDKGameRules C_SDKGameRules
	#define CSDKGameRulesProxy C_SDKGameRulesProxy
#endif

struct LevelInfo_t;

class CSDKGameRulesProxy : public CGameRulesProxy
{
public:
	DECLARE_CLASS( CSDKGameRulesProxy, CGameRulesProxy );
	DECLARE_NETWORKCLASS();
};


class CSDKGameRules : public CTeamplayRules
{
public:
	DECLARE_CLASS( CSDKGameRules, CTeamplayRules );

	virtual bool	ShouldCollide( int collisionGroup0, int collisionGroup1 );

	virtual int		PlayerRelationship( CBaseEntity *pPlayer, CBaseEntity *pTarget );
	virtual bool	IsTeamplay( void ) { return false;	}

#ifdef CLIENT_DLL
	CSDKGameRules();
	DECLARE_CLIENTCLASS_NOBASE(); // This makes datatables able to access our private vars.

#else

	DECLARE_SERVERCLASS_NOBASE(); // This makes datatables able to access our private vars.
	
	CSDKGameRules();
	virtual ~CSDKGameRules();

	virtual bool ClientCommand( const char *pcmd, CBaseEntity *pEdict );
	virtual void RadiusDamage( const CTakeDamageInfo &info, const Vector &vecSrcIn, float flRadius, int iClassIgnore );
	virtual void Think();

	virtual const char *GetChatPrefix( bool bTeamOnly, CBasePlayer *pPlayer );
	virtual void CreateStandardEntities();
	
private:

	void RadiusDamage( const CTakeDamageInfo &info, const Vector &vecSrcIn, float flRadius, int iClassIgnore, bool bIgnoreWorld );

public:

	void CleanMap();

	virtual void Precache( void );
	virtual void ClientDisconnected( edict_t *pClient );
	virtual void PlayerSpawn( CBasePlayer *pPlayer );
	void BeginGame( void );
	void EndGame( void );

	void EnemyKilled( CBaseEntity *player );
	
	//bool m_bGameStarted;
	bool m_bNewLevel;
	int m_iEnemyMaxCount;
	int m_iEnemyAmount;
	int m_iEnemySpawnCount;
	float m_flEnemyIntervalDelay;
	float m_flEnemyRespawnDelay;
	float m_flEnemyIntervalCurrent;
	float m_flEnemyRespawnCurrent;
	
	bool QueryTowerInfo();
	bool GetTowerInfo( int tower, int &cost, int &dmg, int &radius );
	

#endif
	CUtlVector< LevelInfo_t > m_pTowerInfo;
	CNetworkVar( bool, m_bGameStarted );
	CNetworkVar( int, m_iLives );
	CNetworkVar( int, m_iLevel );

};

//-----------------------------------------------------------------------------
// Gets us at the team fortress game rules
//-----------------------------------------------------------------------------

inline CSDKGameRules* SDKGameRules()
{
	return static_cast<CSDKGameRules*>(g_pGameRules);
}


#endif // SDK_GAMERULES_H
